...
void Th_Run  (void const *arg) { while(1) { osThreadYield(); if(arg!=NULL) return; } }
void Th_isr  (void const *arg) { while(1) {                  if(arg!=NULL) return; } }
...
static osThreadId ThId_Running;                         /* Thread ID of the running thread    */
static volatile osThreadId ThId_Isr;                    /* Thread ID returned in the ISR      */
static volatile osPriority ThPr_Isr;                    /* Thread priority returned in the ISR*/
static volatile osStatus   ThSt_Isr;                    /* Status returned in the ISR         */
...
void Th_PrioExec (void const *arg) {
  ...
  for (i = 0; i < 7; i++) {
    if (ExecArr[i] == 0) {
      ExecArr[i] = *(osThreadId const *)arg;
      break;
    }
  }
}
