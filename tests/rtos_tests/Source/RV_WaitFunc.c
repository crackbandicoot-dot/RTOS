...
static osMutexId       G_WaitFunc_MutexId;              /* Global mutex id                    */
static osThreadId      G_WaitFunc_ThreadId;             /* Global thread id                   */
static osSemaphoreId   G_WaitFunc_SemaphoreId;          /* Global semaphore id                */
static osMessageQId    G_WaitFunc_MessageQId;           /* Global message queue id            */
static osMailQId       G_WaitFunc_MailQId;              /* Global mail queue id               */
...
#if (osFeature_Wait)  
  Lim_OsWait[0]               = (uint32_t) (TickCyc - (TickCyc * ACCURACY_OS_WAIT           / 1000));
  Lim_OsWait[1]               = (uint32_t) (TickCyc + (TickCyc * ACCURACY_OS_WAIT           / 1000));
#endif

  Lim_OsDelay[0]         = (uint32_t) (TickCyc - (TickCyc * ACCURACY_OS_DELAY          / 1000));
  Lim_OsDelay[1]         = (uint32_t) (TickCyc + (TickCyc * ACCURACY_OS_DELAY          / 1000));

  Lim_OsSignalWait[0]    = (uint32_t) (TickCyc - (TickCyc * ACCURACY_OS_SIGNAL_WAIT    / 1000));
  Lim_OsSignalWait[1]    = (uint32_t) (TickCyc + (TickCyc * ACCURACY_OS_SIGNAL_WAIT    / 1000));

  Lim_OsMutexWait[0]     = (uint32_t) (TickCyc - (TickCyc * ACCURACY_OS_MUTEX_WAIT     / 1000));
  Lim_OsMutexWait[1]     = (uint32_t) (TickCyc + (TickCyc * ACCURACY_OS_MUTEX_WAIT     / 1000));

  Lim_OsSemaphoreWait[0] = (uint32_t) (TickCyc - (TickCyc * ACCURACY_OS_SEMAPHORE_WAIT / 1000));
  Lim_OsSemaphoreWait[1] = (uint32_t) (TickCyc + (TickCyc * ACCURACY_OS_SEMAPHORE_WAIT / 1000));

  Lim_OsMessageWait[0]   = (uint32_t) (TickCyc - (TickCyc * ACCURACY_OS_MESSAGE_WAIT   / 1000));
  Lim_OsMessageWait[1]   = (uint32_t) (TickCyc + (TickCyc * ACCURACY_OS_MESSAGE_WAIT   / 1000));
                                                                                              
  Lim_OsMailWait[0]      = (uint32_t) (TickCyc - (TickCyc * ACCURACY_OS_MAIL_WAIT      / 1000));
  Lim_OsMailWait[1]      = (uint32_t) (TickCyc + (TickCyc * ACCURACY_OS_MAIL_WAIT      / 1000));
}
...
static void WaitFunc_Th_MutexLock (void const __attribute__((unused)) *arg) {
  ...
  ASSERT_TRUE (osMutexRelease (G_WaitFunc_MutexId) == osOK);
  osSignalWait (0x01, osWaitForever);
}
...
void TC_MeasureOsMutexWaitTicks (void) {
  ...
  if (CycCntRunning) {
    ...
    ASSERT_TRUE (osSignalSet(G_WaitFunc_ThreadId, 0x01) >= 0);
    osDelay(10);
