...
/* Definitions for TC_SemaphoreCheckTimeout */
...
void Semaphore_IRQHandler (void) {
  ...
  switch (ISR_ExNum) {
    ...
    case 3: NumTokens_Isr = osSemaphoreWait    (SemId_Isr, osWaitForever); break;
    case 4: SemSt_Isr     = osSemaphoreRelease (SemId_Isr);                break;
    case 5: SemSt_Isr     = osSemaphoreDelete  (SemId_Isr);                break;
  }
}
 ...
 *----------------------------------------------------------------------------*/
static void Th_TestSemaphore (void const *arg) {
  uint32_t const par = *(uint32_t const *)arg;
  int32_t  sem;
 ...
 *----------------------------------------------------------------------------*/
static void Th_SemaphoreWait (void const *arg) {
  uint32_t const ticks = *((uint32_t const *)arg);
...
void TC_SemaphoreObtainCounting (void) {
  ...
  if (id != NULL) {
    ...
    ASSERT_TRUE (osSemaphoreRelease (id) == 0);
...
void TC_SemaphoreWaitTimeout (void) {
  ...
  G_SemaphoreId = osSemaphoreCreate (osSemaphore (Sem_TestWaitTimeout), 1);
  ASSERT_TRUE (G_SemaphoreId != NULL);
...
void TC_SemaphoreCheckTimeout (void) {
  ...
  /* Create a semaphore object */
  G_SemaphoreId = osSemaphoreCreate (osSemaphore (Sem_CheckTimeout), 0);
  ASSERT_TRUE (G_SemaphoreId != NULL);
  ...
  if (G_SemaphoreId != NULL) {    
    ...
    /* Create wakeup thread to release a semaphore after 10 ticks */
    ASSERT_TRUE (osThreadCreate(osThread(Th_SemaphoreWakeup), NULL) != NULL);
    t_10 = osKernelSysTick();
    avail = osSemaphoreWait (G_SemaphoreId, 100);
    ...
    ASSERT_TRUE (osThreadCreate(osThread(Th_SemaphoreWakeup), NULL) != NULL);
    t_10 = osKernelSysTick();
    avail = osSemaphoreWait (G_SemaphoreId, osWaitForever);
    t_10 = osKernelSysTick() - t_10;
    ASSERT_TRUE (avail == 1);
    ASSERT_TRUE (t_min < t_10);
    ASSERT_TRUE (t_10 < t_max);
  ...
  if (SemId_Isr == NULL) {
    ...
    if (SemId_Isr != NULL) {
      ...
      if (NumTokens_Isr == -1) {
        ...
        ISR_ExNum = 3; /* Test: osSemaphoreWait (with infinite time-out) */
        NumTokens_Isr = 0;
        SetPendingIRQ((IRQn_Type)0);
        ASSERT_TRUE (NumTokens_Isr == -1);
        ...
        if (NumTokens_Isr == -1) {
          ...
          ISR_ExNum = 4; /* Test: osSemaphoreRelease */
          SemSt_Isr = osErrorOS;
          SetPendingIRQ((IRQn_Type)0);
      ...
      ISR_ExNum = 5;  /* Test: osSemaphoreDelete */
      SemSt_Isr = osOK;
      SetPendingIRQ((IRQn_Type)0);
      ASSERT_TRUE (SemSt_Isr == osErrorISR);
      
      ASSERT_TRUE (osSemaphoreDelete (SemId_Isr) == osOK);      
    }
  }
