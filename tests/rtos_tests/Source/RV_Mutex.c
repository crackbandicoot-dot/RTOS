...
static osMutexId   G_MutexId;                  /* Global mutex id                    */
static osThreadId  G_Mutex_ThreadId;           /* Global thread id                   */
 ...
 *----------------------------------------------------------------------------*/
static void Th_MutexLock (void const *arg) {
  const osThreadId *ctrl_id = (const osThreadId *)arg;
  osEvent    evt;
 ...
 *----------------------------------------------------------------------------*/
void Th_LowPrioJob (void const *arg) {
  osThreadId const *ctrl_id = (osThreadId const *)arg;
  osStatus    stat;
  uint32_t    i;
...
void TC_MutexCheckTimeout (void) {
  ...
  G_MutexId = osMutexCreate (osMutex (Mutex_CheckTimeout));
  ASSERT_TRUE (G_MutexId != NULL);
  ...
  if (G_MutexId != NULL) {    
    ...
    ASSERT_TRUE (osThreadCreate(osThread(Th_MutexWakeup), NULL) != NULL);
    ...
    ASSERT_TRUE (osThreadCreate(osThread(Th_MutexWakeup), NULL) != NULL);
    t_10 = osKernelSysTick();
    stat = osMutexWait (G_MutexId, osWaitForever);
    t_10 = osKernelSysTick() - t_10;
    ASSERT_TRUE (stat == osOK);
    ASSERT_TRUE (t_min < t_10);
    ASSERT_TRUE (t_10 < t_max);
...
void TC_MutexOwnership (void) {
  ...
  if (G_MutexId != NULL) {
    ...
    if (id[0] != NULL) {
      ...
      ASSERT_TRUE (osSignalSet(id[0], 0x01) >= 0);
      osDelay(10);
